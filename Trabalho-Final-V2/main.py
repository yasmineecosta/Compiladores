# main.py (Ajustado para focar no Front-End)
...
# from tac_generator import TACGenerator # Comentado para não dar erro nesta fase
    ...
    try:
        ...
        for token in token_stream.tokens:
            if token.type == Token.EOF:
                break
            # Garante que o nome do token seja recuperado corretamente
            token_name = symbolic_names[token.type] if token.type < len(symbolic_names) and symbolic_names[token.type] != "<INVALID>" else literal_names[token.type]
        ...
        if analisador_semantico.erros:
            ...
            for erro in sorted(analisador_semantico.erros):
                print(f"  -> {erro}")
        else:
            ...
            # A AST agora é construída no nó raiz
            ast_string = ast_builder.get_ast_string(tree)
            print(ast_string)

            # --- Geração do Código de Três Endereços (TAC) ---
            # O código abaixo está desativado para a entrega do front-end
